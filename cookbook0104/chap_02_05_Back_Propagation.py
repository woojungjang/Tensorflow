...
# Create data
x_vals = np.random.normal(1, 0.1, 100) #입력 데이터(평균, 표준편차, 요소갯수)
y_vals = np.repeat(10., 100)#출력데이터(10.0을 100번 반복)
...
print('\n회귀예제')
for i in range(100):
    rand_index = np.random.choice(100) #rand_index: 0 이상 100미만에서 요소1개 추출한 색인
    rand_x = [x_vals[rand_index]] #입력 데이터중 임의의 1개
    rand_y = [y_vals[rand_index]] #출력 데이터중 임의의 1개
    sess.run(train_step, feed_dict={x_data: rand_x, y_target: rand_y})
    if (i+1)%25==0:
        print('Step #' + str(i+1) + ' A = ' + str(sess.run(A)))
        print('Loss = ' + str(sess.run(loss, feed_dict={x_data: rand_x, y_target: rand_y})))

# Classification Example: 분류문제
# We will create sample data as follows:
